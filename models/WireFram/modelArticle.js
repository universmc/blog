...
async function generateImage(subject) {
  try {
    ...
    const fileName = `output/build/image_${subject}_${date}.webp`;
...
async function generateDocumentation(subject) {
  ...
  const outputFilePath = `output/build/how-to_${subject}_${new Date().toISOString().replace(/[-:TZ]/g, "")}.md`;
...
<body>
  ...
  const htmlFileName = `output/build/${subject}_howto_${new Date().toISOString().replace(/[-:TZ]/g, "")}.html`;
